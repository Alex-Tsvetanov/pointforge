...
// на този интерфейс.
//
// Трите пътя за изпълнение на листата — скаларен, пакетен и SIMD — са на
// разположение през един и същ метод и се избират с параметър. Така една и
// съща заявка се изпълнява по всеки начин върху едни и същи данни, което е
// условието сравнението между тях да е измерване, а не твърдение.
...
class KdTree {
public:
    // Дължина на блока в пакетния и SIMD пътя. Кратна на ширината на
    // векторния регистър при float за AVX (8) и за SSE (4).
